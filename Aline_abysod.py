...
class ALINE():
    ...
    # Vowels
        ...
        '\u0329': {'syllabic': 'plus', 'supplemental': True},
    }
    ...
    def alignments(self, src, tar, score_only=False):
            ...
            s_mat = np_zeros((src_len + 1, tar_len + 1), dtype=float)
...
def demo():
        ...
        for pair in data:
            cmd = ALINE()
            sim_dict[f"{pair[0]} ~ {pair[1]}"] = cmd.sim(pair[0], pair[1])
...
def demo_drugs():
    import pandas as pd
    df = pd.read_csv(r"data\FDA_OB_phone.csv", encoding='utf-8')
    df = df["phonemized_name"]
    df_first_50 = df[:10]
    drug_main_dict = {}
    drug_main_dict_sorted = {}
    
    for drug1 in df_first_50:
        drug_main_dict[drug1] = {}
        for drug2 in df:
            cmd = ALINE()
            drug_main_dict[drug1][drug2] = cmd.sim(drug1, drug2)
        drug_main_dict_sorted[drug1] = sorted(drug_main_dict[drug1].items(), key=lambda x: x[1], reverse=True)
    
    for drug in drug_main_dict_sorted:
        print(f"\n                          *** Main Drug: {drug}***".format(drug))
        print("{:<70} | {:<30}".format("Compared Drug", "Similarity"))
        print("-" * 78)
        
        print("Top 5:")
        for pair in drug_main_dict_sorted[drug][:5]:
            print("     {:<70} | {:<30.3f}".format(pair[0], pair[1]))
...
def drugs_matrix():
    ...
    df = pd.read_csv(r"data\FDA_OB_phone.csv", encoding='utf-8-sig')
    df = df[["Trade_Name", "Phonemized"]]
    drug_dict = df.set_index('Trade_Name')['Phonemized'].to_dict()

    cmd = ALINE()

    # Open the text file for writing. 'a' mode appends to the file without truncating it.
    with open(r"data\results.txt", "a") as file:
        # Check if file is empty to write headers
        file.seek(0, 2)  # Move the cursor to the end of the file
        if file.tell() == 0:  # If file is empty, write headers
            file.write("Drug1;Drug2;Score\n")
        
        for drug1, drug2 in combinations(drug_dict.keys(), 2):
            if "ERROR" in [drug_dict[drug1], drug_dict[drug2]]:
                similarity_score = 0
            elif drug_dict[drug1] == drug_dict[drug2]:
                similarity_score = 1
            else:
                similarity_score = cmd.sim(drug_dict[drug1], drug_dict[drug2])*100
                similarity_score = round(similarity_score, 1)
            
            # Write each result directly to the file
            file.write(f"{drug1};{drug2};{similarity_score}\n")
